...
var fn = {
  ...
  */
  shouldDataUpdate: function shouldDataUpdate(thisProps) {
    var nextProps = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    var propsIndex = arguments[2];
  ...
  */
  imutableReducer: function imutableReducer(state, action, type, callbackBefore) {
    callbackBefore && callbackBefore();
    var data = {};
    if (state.toJS) {
      if (type == "request") {
        return state.mergeDeep(_immutable2.default.fromJS(action));
      }
      if (type == "receive") {
        return state.merge(_immutable2.default.fromJS(action));
      }
    } else {
      data = Object.assign({}, state, action);
      var i_state = _immutable2.default.fromJS(data);
      return i_state;
    }
  },
